...
(async () => {
  ...
  if (!packageJsonOfConfig.engines.node)
    packageJsonOfConfig.engines.node = `>=${process.versions.node}`;

  const templateContent = JSON.parse(
    base.getTemplate(__dirname, 'package.json', {
